...
function normalizeIdentifyResult(result) {
  ...
  return {
    ...
    confidence: normalizeConfidence(result.confidence, result.status),
    name: normalizedName,
...
function normalizeConfidence(confidence, status) {
  const text = stringify(confidence, "").toLowerCase();
  const normalizedStatus = stringify(status, "").toLowerCase();
  if (normalizedStatus === "unknown" || /无法识别|无法|看不出|没有主体|没有生物|unknown/.test(text)) {
    return "无法识别";
  }
  if (normalizedStatus === "identified" || /准确|明确|确定|较明确|高|identified/.test(text)) {
    return "准确";
  }
  return "模糊";
}
