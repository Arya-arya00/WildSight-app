...
function identifyUserPrompt() {
  return [
    ...
    "- 不要编造出处，不要编造冷知识，不要把推测写成事实。",
    "- 如果图片只能判断到大类，就只写这个大类确定成立的事实，并说明“不一定精确到种”。",
    "- 如果某个有趣点无法确认，就不要写冷知识；facts 可以只有 2 条。",
    "",
    "识别流程要求：",
    ...
    "",
    "facts 结构：",
    "1. title 用“1️⃣ 它是谁：……”；text 讲它是什么、怎么认、身体结构特点。优先抓照片里看得见的特征。",
    "2. title 用“2️⃣ 怎么生活：……”；text 讲习性，比如昼夜节律、捕食方式、食性、栖息环境、社交/独居、游动/伪装方式。",
    "3. 如果且仅如果有可靠、有趣、可验证的知识点，title 用“3️⃣ 冷知识：……”；text 讲一个真正有记忆点的事实，优先从繁殖、发育、呼吸、捕食、伪装、共生、清洁行为、防御机制里选。没有就不要写第三条。",
    "",
...
function normalizeIdentifyResult(result) {
  ...
  return {
    ...
    facts: facts.map((fact) => ({
      title: stringify(fact?.title, "观察提示"),
...
function imageTimeoutMs() {
  return Number(process.env.OPENAI_IMAGE_TIMEOUT_MS || 22000);
}
